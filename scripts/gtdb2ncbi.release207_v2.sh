...
# 	mkdir -p $GTDBTK_DB
# fi
# if ! test -f $GTDBTK_DB/ar53_metadata_r*.tsv || [ `ls $GTDBTK_DB | grep $'ar53_metadata_r.*.tsv' | sort -r | head -1 | awk -F'[/.]' '{print $(NF-1)}' | cut -c1-6` -ne `date +'%Y%m'` ]
# then
# 	echo "#### DATABASE CHECK: Could not find ${ar_prefix}_metadata in $GTDBTK_DB. Downloading... ####"
# 	cd $GTDBTK_DB
# 	rm -rf $GTDBTK_DB/ar53_metadata_r*.tsv
...
# 	name=`basename $GTDBTK_DB/ar53_metadata_r*.tsv .tsv`
# 	mv $GTDBTK_DB/ar53_metadata_r*.tsv $name.$current_day.tsv
# fi
# if ! test -f $GTDBTK_DB/bac120_metadata_r*.tsv || [ `ls $GTDBTK_DB | grep $'bac120_metadata_r.*.tsv' | sort -r | head -1 | awk -F'[/.]' '{print $(NF-1)}' | cut -c1-6` -ne `date +'%Y%m'` ]
# then
# 	echo "#### DATABASE CHECK: Updating ${bac_prefix}_metadata in $GTDBTK_DB. Downloading... ####"
# 	rm -rf $GTDBTK_DB/bac120_metadata_r*.tsv
...
# 	name=`basename $GTDBTK_DB/bac120_metadata_r*.tsv .tsv`
# 	mv $GTDBTK_DB/bac120_metadata_r*.tsv $name.$current_day.tsv
# fi
#
#

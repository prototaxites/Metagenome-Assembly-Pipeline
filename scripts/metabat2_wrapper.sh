...
# METABAT2 WRAPPER #
usage="SYNOPSIS: metabat_wrapper.sh [OPTIONS]\n\
OPTIONS:\n\
...
if [ "$assembly" == "" ]
then
	echo -e "#### ERROR --- METABAT2 WRAPPER: Assembly file required. ####"
	if test -d $logout && test -f $logout/mag_pipe_progress.log
	then
		echo -e "#### ERROR --- METABAT2 WRAPPER: Assembly file not provided. ####" >> $logout/mag_pipe_progress.log
		echo -e "$prefix\tmetabat2" >> $ERROR_OUT
	fi
	exit 1
fi
...
###################################################################
mkdir -p $outdir
rm -rf $outdir/metabat2_complete
cd $outdir
...
then
	if [ "$depth" == "" ] && [ "$reads" == "" ]
	then
		echo -e "#### ERROR --- METABAT2 WRAPPER: Either depth file or long read file required. ####" >> $logout/mag_pipe_progress.log
		echo -e "#### ERROR --- METABAT2 WRAPPER: Either depth file or long read file required. ####"
		echo -e "$prefix\tmetabat2" >> $ERROR_OUT
		exit 1
	elif [ "$depth" == "" ] && [ "$reads" != "" ]
	then
		...
		then
			...
			then
				echo "#### METABAT2 WRAPPER: Mapping long reads to assembly ####" >> $logout/mag_pipe_progress.log
				echo "#### METABAT2 WRAPPER: Mapping long reads to assembly ####"
				minimap2 \
					-a -t $threads \
					-x map-hifi \
					$assembly $reads \
			...
			echo "#### METABAT2 WRAPPER: Calculating contig read depth ####" >> $logout/mag_pipe_progress.log
			echo "#### METABAT2 WRAPPER: Calculating contig read depth ####" 
	...
	echo "#### METABAT2 WRAPPER: Binning assembly ####" >> $logout/mag_pipe_progress.log
	echo "#### METABAT2 WRAPPER: Binning assembly ####" 
	mkdir -p $outdir/bin
	singularity exec -B /lustre,/nfs \
		$LOCAL_IMAGES/metabat.sif \
			metabat2 \
				...
				-a $depth \
				-o $outdir/bin/bin
	if ! test -d $outdir/bin || [ `ls $outdir/bin/*.fa| wc -l` -eq 0 ]
	then
		echo -e "$prefix\tmetabat2" >> $ERROR_OUT
		echo -e "#### ERROR --- METABAT2 WRAPPER: No output produced by METABAT2 ####" >> $logout/mag_pipe_progress.log
		echo -e "#### ERROR --- METABAT2 WRAPPER: No output produced by METABAT2 ####"
		exit 1
	fi
...
contigs2bin.sh \
	...
	fa
touch $outdir/metabat2_complete

###################################################################

extension=fa
binning_program=metabat2
refinement_tool=raw
if [ "$eval" == "TRUE" ]
then
	if ! test -f $outdir/binstats_complete
	then
		echo "#### METABAT2 WRAPPER: Running Binstat Wrapper for binned assemblies ####" >> $logout/mag_pipe_progress.log
		echo "#### METABAT2 WRAPPER: Running Binstat Wrapper for binned assemblies ####"
...
then
	...
	then
		echo "#### METABAT2 WRAPPER: Running DASTOOL wrapper on binned assemblies ####" >> $logout/mag_pipe_progress.log
		echo "#### METABAT2 WRAPPER: Running DASTOOL wrapper on binned assemblies ####"
		# bsub -n$threads -q long -R"span[hosts=1]" \
...
echo "#### METABAT2 WRAPPER: Complete ####" >> $logout/mag_pipe_progress.log
echo "#### METABAT2 WRAPPER: Complete ####"
touch $outdir/metatbat2_complete
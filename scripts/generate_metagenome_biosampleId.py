...
def copy_checklist_items(field_dict, parent_dict, child_dict):
    ...
    for parent_key, parent_val in parent_dict.items():
        if parent_key not in child_dict.keys():
            if parent_key == "sex":
                parent_val[0] = parent_val[0].lower()
                if "hermaphrodite" in parent_val[0]:
                    parent_val[0] = "hermaphrodite"
                elif "sexual morph" in parent_val[0]:
                    parent_val[0] = "other"
                child_dict["host sex"] = parent_val
            elif parent_key == "lifestage":
                child_dict["host life stage"] = parent_val
            elif parent_key =="geographic location (latitude)" or parent_key == "geographic location (longitude)":
                try:
                    float(parent_val[0])
                except:
                    print("value a string")
                else:
                    parent_val[0] = str("{:.2f}".format(float(parent_val[0])))
